...
class Base extends Model
{
     ...
     * Description 查找单条数据
     */
    public static function getOnes($where, $field = '*' ,$order = ['id','desc'])
    {
    ...
    public static function __callStatic($method, $parameters)
    {
        ...
        if($method ==  'getOne'){
            if(isset($parameters[0][self::$pk])){
                $cache_key = self::getCacheKey($parameters[0][self::$pk]);
                $res = json_decode(Redis::get($cache_key),true);
                if(empty($res)){
                    $res = self::getOnes(...$parameters);
                    Redis::set($cache_key,$res,(new self)->cache_ttl);
                }
                return $res;
            }
        }
